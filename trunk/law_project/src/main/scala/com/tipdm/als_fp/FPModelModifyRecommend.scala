...
/**
  * FP 模型的关联规则修正ALS的推荐结果
  * //@Author: fansy 
  * //@Time: 2018/10/26 14:51
  * //@Email: fansy1990@foxmail.com
  */
object FPModelModifyRecommend {
  ...
  def generateRules(sc: SparkContext, sqlContext: SQLContext, trainTable: String,
    ...
    val data = sqlContext.sql("select " + uidCol + " , " + pidCol + " from " + trainTable).distinct.groupBy(uidCol)
      .agg(collect_set(col(pidCol)) as URL_SETS_COLUMN).filter(size(col(URL_SETS_COLUMN)) > 1)
  ...
  /**
    * 推荐
    * 遍历validate_train 数据，针对每个validate中评价过的数据，按照评价的个数分别从 rules_ordered 的对应位置（解析见下）进行往下遍历，得到recNum个值（这些值不能在train中出现，排除用户已经评价过的数据）后返回；
    ...
    *
    * @param validate_visited
    * @param train_visited
    * @param rules : 和 validate_visited 中url个数匹配的规则
    * @param recNum
    * @return
    */
  def getRecommend(validate_visited: Set[Int], train_visited: Set[Int], rules: Array[MyRule], recNum: Int): Array[Rating] = {
    val buff = new ArrayBuffer[Rating]()
    for (rule <- rules) {
      if (rule.antecedent.&(validate_visited).size == rule.antecedent.size) {
        //  前缀全匹配
        if (!train_visited.contains(rule.consequent)) {
          //  推荐值不应出现在 train_visited中
  ...
  /**
    ...
    * 3. 遍历validate_train 数据，针对每个validate中评价过的数据，按照评价的个数分别从 rules_ordered 的对应位置（解析见下）进行往下遍历，得到recNum个值（这些值不能在train中出现，排除用户已经评价过的数据）后返回；
    *
    * 对应位置： 如果当前rules_ordered里面的antecedent的最长为4，那么当validate中的数据长度大于或等于4，则从头开始遍历，如果等于3，那么则从rules_ordered的 rule.antecedent.length为3的地方开始匹配；
    *
    * @param rules_origin
    ...
    * @return
    */
  def getWithRecommend(rules_origin: RDD[AssociationRules.Rule[Long]], train: RDD[(Int, Set[Int])], validate: RDD[(Int, Set[Int])], recNum: Int)
  : RDD[(Int, (Array[Int], Array[Rating]))] = {
    ...
    val rules_ordered = rules_origin.collect().sorted(length_confidence)
      .map((x: AssociationRules.Rule[Long]) => MyRule(x.antecedent.toSet.map((y: Long) => y.toInt), x.consequent.head.toInt, x.confidence))
    ...
    validate.leftOuterJoin(train)
      .map { x =>
        val url_size = if (x._2._1.size >= antecedent_length_count.head._1) antecedent_length_count.head._1 else x._2._1.size
        (x._1, (x._2._1.toArray, getRecommend(x._2._1, x._2._2.getOrElse(Set[Int]()), all_rules_ordered(antecedent_length_count.length - url_size), recNum)))
      }
  }
  ...
  def main(args: Array[String]): Unit = {
    ...
    //  3. 推荐
    val train = sqlContext.sql("select * from " + trainTable).select(uidCol, pidCol).groupBy(uidCol)
      .agg(collect_set(pidCol)).rdd.repartition(24).map(row => (row.getLong(0).toInt, (row.getSeq[Long](1)).map(_.toInt).toSet))
    val validate = sqlContext.sql("select * from " + validateTable).select(uidCol, pidCol).groupBy(uidCol)
      .agg(collect_set(pidCol)).rdd.repartition(8).map(row => (row.getLong(0).toInt, (row.getSeq[Long](1)).map(_.toInt).toSet))
    val recommended = getWithRecommend(rules, train, validate, recNum)
    //  4. 评估
    val (p, r, f) = evalute(recommended)
    println("precision :" + p + " \t recall: " + r + " \t F1:" + f)
