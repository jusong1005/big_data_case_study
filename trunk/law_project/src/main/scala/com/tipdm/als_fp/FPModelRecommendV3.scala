...
/**
  ...
  * 针对验证提出不同的思路：
  * 主要思想：针对所有用户访问一个URL后进行推荐，同时限定用户访问的时间（用户访问时间内访问的URL才用于验证）；
  * 以此推荐结果来验证Precision和Recall
  * //@Author: fansy 
  * //@Time: 2018/10/30 15:53
  * //@Email: fansy1990@foxmail.com
  */
object FPModelRecommendV3 {
  ...
  /**
    * 处理validate数据，只保留，每个用户6分钟内访问的记录，并返回（这个用户第一次访问的记录，后面访问的记录（可能没有访问））；
    * 不排除掉前后两个URL访问间隔为0的记录；
    *
    * @param sqlContext
    * @param validateTable
    * @param uidCol
  ...
  def main(args: Array[String]): Unit = {
    ...
    // 3. 推荐, 只保留 validateTable中每个用户访问后，6分钟内的访问记录；然后推荐1个；所以保留每个用户多个访问URL，最少访问1个URL
    ...
    for (recNum <- recNums; valid_minute <- valid_minutes) {
      ...
      println(recNum + " \t " + valid_minute + " \t " + p.formatted("%.4f") + "% \t " + r.formatted("%.4f") + "% \t " + f.formatted("%.4f") + "% \t " + size + " \t " + p_sum + " \t " + r_sum + " \t " + f_sum)
    }
