...
**【成果结晶：深色科技风用户画像平台】**
我们对前端进行了全面重构，采用深色科技风大屏设计。主页面板 `http://localhost:8001/user_profile/` 包含 KPI 指标卡片、用户搜索（支持示例用户快捷选择和随机按钮）、ECharts 标签知识图谱、用户画像雷达图及标签分组展示。

![前端用户画像-深色科技风大屏](./项目总结/22_前端用户画像-标签拓扑雷达图.png)
_(图：重构后的深色科技风用户画像平台。顶部 KPI 展示核心指标，左侧为基于真实 9 大标签的知识图谱，右侧为用户画像结果——包含 ECharts 雷达图和彩色标签分组，直观呈现用户在消费、业务、预测三大维度的特征)_

**【实时数据监控大屏】**
通过顶部导航栏的”实时大屏”入口进入 `http://localhost:8001/user_profile/screen`，展示从 Redis 读取的实时订单数据和从 MySQL 统计的标签分布，每 10 秒自动刷新。

![实时数据监控大屏](./项目总结/23_前端用户画像-明文总览列表.png)
_(图：实时数据监控大屏。左侧为各标签用户分布柱状图和业务品牌占比饼图（MySQL），中间为按小时订单趋势柱状图+营业额折线图（Redis），右侧为消费水平饼图和标签滚动列表。顶部 KPI 展示总订单 22,000、总营业额 ¥1,205,992、有效订单 22,000 等实时指标)_
...
### 2. Spring Boot 可视化服务启动
- **子工程**：user_profile_visualization
- **方法**：在 VS Code 中进入终端 2，以 JDK 8 启动：
  ```powershell
  $env:JAVA_HOME="D:\java\jdk1.8.0_202"; mvn clean spring-boot:run
  ```
- **访问地址**：`http://localhost:8001/user_profile/`
...
```bash
sh /root/start_services.sh
```

启动 Hadoop（HDFS + YARN）、Hive Metastore、Spark、Kafka、Zookeeper、Redis 等底层基础设施。

#### 1.2 Hive Metastore 初始化（仅首次）

```bash
schematool -dbType mysql -initSchema
```

初始化 Hive 元数据库（MySQL），只需在首次部署时执行一次。

#### 1.3 Spark 连接 Hive 配置同步（仅首次）

```bash
cp /export/servers/hive-1.2.1/conf/hive-site.xml /export/servers/spark-1.6.3/conf/
```

使 Spark 能读取 Hive Metastore 的 MySQL 连接信息，否则 Spark 会用内嵌 Derby 找不到表。
...
# 检查指定端口占用（如 Spark UI 4040）
lsof -i:4040

# 杀掉占用端口的残留进程
kill -9 <PID>

# 检查 Kafka/Zookeeper/Redis 端口是否正常监听
netstat -tlnp | grep -E '2181|9092|6379'
```
...
```bash
/export/servers/kafka_2.10-0.10.2.2/bin/kafka-topics.sh \
...
#### 2.3 向 Kafka 灌入模拟订单数据

```bash
cat /root/mock_orders.txt | /export/servers/kafka_2.10-0.10.2.2/bin/kafka-console-producer.sh \
  --broker-list 192.168.2.162:9092 --topic order_topic
```

#### 2.4 启动 Spark Streaming 实时消费

```bash
spark-submit \
  --driver-class-path /export/servers/hive-1.2.1/lib/mysql-connector-java-5.1.45.jar \
  --class com.tipdm.scala.chapter_3_7_streaming.KafkaStream \
  --master local[*] \
  /export/data/big_data_case_study_project/user_profile_project-1.0.jar
```

启动后每 10 秒消费一批 Kafka 数据，统计结果实时写入 Redis。
...
get totalcost
get totalOrders
get totalValidOrders
get increase_cost
get increase_order
```

### 三、离线 ETL：Hive 建表与 Spark 清洗

#### 3.1 上传原始 CSV 数据到虚拟机

```bash
# 在虚拟机上创建数据目录
mkdir -p /export/data/raw_data/

# 从本地通过 SCP 上传（在宿主机执行）
scp media_index_test.csv root@192.168.2.162:/export/data/raw_data/
scp mediamatch_userevent.csv root@192.168.2.162:/export/data/raw_data/
scp mediamatch_usermsg.csv root@192.168.2.162:/export/data/raw_data/
scp mmconsume_billevents.csv root@192.168.2.162:/export/data/raw_data/
scp order_index_test.csv root@192.168.2.162:/export/data/raw_data/
```

#### 3.2 Hive ODS 层建表 + 数据装载

```bash
hive -f /root/create_ods_fixed.sql
```

该脚本执行以下操作：
- 创建 5 张 ODS 层宽表（`media_index_3m`、`mediamatch_userevent_3m`、`mediamatch_usermsg_3m`、`mmconsume_billevents_3m`、`order_index_v3`）
- 通过 `LOAD DATA LOCAL INPATH` 将 CSV 文件灌入 HDFS

> 注意：分号分隔的表使用 `'\073'` 八进制转义，逗号分隔的表使用 `','`。
...
# 检查各表数据量
hive -e "select count(*) from media_index_3m;"
hive -e "select count(*) from mediamatch_userevent_3m;"
hive -e "select count(*) from mediamatch_usermsg_3m;"
hive -e "select count(*) from mmconsume_billevents_3m;"
hive -e "select count(*) from order_index_v3;"

# 抽样查看数据
hive -e "select * from media_index_3m limit 5;"
```
...
# 查看某张表的 Parquet 分区文件
hadoop fs -ls /user/hive/warehouse/mediamatch_userevent_1d/
```

#### 3.5 Spark ETL 离线清洗（ODS → DWD）
...
将 ODS 层 5 张原始表清洗后写入 DWD 层 5 张明细表（Parquet 格式）。
...
依次计算 9 大用户画像标签（消费内容、电视消费水平、宽带消费水平、宽带产品带宽、用户是否挽留、业务品牌、电视入网程度、宽带入网程度、销售品名称），通过 JDBC 写入 MySQL `zjsm.user_label` 表。

#### 5.2 MySQL 数据验证

```bash
# 登录 MySQL
mysql -u root -p123456 -h 192.168.2.162

# 在 MySQL 中执行
USE zjsm;
SELECT parent_label, COUNT(*) AS 总记录数 FROM user_label GROUP BY parent_label;
SELECT * FROM user_label LIMIT 10;
```

### 六、ELK 日志监控

#### 6.1 启动 Elasticsearch

```bash
# ES 禁止 root 启动，必须切换用户
su - elastic -c "/export/servers/elasticsearch-6.3.2/bin/elasticsearch -d"

# 验证 ES 是否就绪（必须返回 JSON 才算成功）
...
# 访问地址：http://192.168.2.162:5601
```

> 首次部署需修改 `kibana.yml`：
> ```bash
> sed -i 's/#server.host: "localhost"/server.host: "0.0.0.0"/' \
>   /export/servers/kibana-6.3.2/config/kibana.yml
> ```

#### 6.3 Kibana 集群健康检查

在 Kibana Dev Tools (`http://192.168.2.162:5601/app/kibana#/dev_tools/console`) 中执行：
...
#### 7.1 启动 Filebeat（日志采集）

```bash
cd /export/servers/filebeat-6.3.2-linux-x86_64/
nohup ./filebeat -e -c filebeat.yml &
```

监听 Nginx `access.log` / `error.log` 和自定义日志文件，实时推送至 Elasticsearch。

#### 7.2 启动 Metricbeat（系统指标采集）

```bash
cd /export/servers/metricbeat-6.3.2-linux-x86_64/

# 启用 system 和 nginx 监控模块
./metricbeat modules enable system nginx

# 启动
nohup ./metricbeat -e &
```

#### 7.3 Logstash CSV 数据清洗

```bash
# 抽样前 1000 条数据进行验证
head -1001 /export/data/raw_data/mediamatch_userevent.csv > /tmp/sample.csv

# 启动 Logstash 执行 CSV 过滤清洗
/export/servers/logstash-6.3.2/bin/logstash -f /path/to/your_config.conf
```
...
```
步骤 1: sh /root/start_services.sh          # 启动集群
步骤 2: schematool -dbType mysql -initSchema # Hive 初始化（仅首次）
步骤 3: hive -f create_ods_fixed.sql         # ODS 建表 + 装载数据
步骤 4: spark-submit ... DataProcess ...     # ETL 清洗 ODS → DWD
步骤 5: sh /root/run_svm.sh                  # SVM 模型训练 + 预测
步骤 6: sh /root/run_labels.sh               # 9 大画像标签 → MySQL
步骤 7: cat mock_orders.txt | kafka-console-producer.sh ...  # Kafka 灌入订单
步骤 8: spark-submit ... KafkaStream ...     # 实时流消费 → Redis
步骤 9: su - elastic -c "... elasticsearch -d"  # 启动 ES
步骤10: nohup kibana &                       # 启动 Kibana
步骤11: nohup filebeat &                     # 启动日志采集
步骤12: nohup metricbeat &                   # 启动指标采集
```

> 注意：步骤 7-8（实时流）和步骤 9-12（ELK 监控）相互独立，可并行执行。步骤 1-6 必须严格按顺序执行。